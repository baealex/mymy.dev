    ...
    io.on('connection', (socket) => {
        socket.on(SOCKET_EVENT_NAME.CODE_RUNNER, ({ language, source }: CodeRunnerEventParams) => {
            ...
            try {
                ...
                switch (language) {
                    case 'c':
                        socket.emit(SOCKET_EVENT_NAME.CODE_RUNNER_RESULT, CodeRunnerResultEventParams({
                            data: runCode({
                                filename,
                                env: 'cpp',
        ...
        socket.on(SOCKET_EVENT_NAME.GET_GITHUB_RAW, async ({ raw }: GetGitHubRawEventParams) => {
            ...
            try {
                ...
                socket.emit(SOCKET_EVENT_NAME.GET_GITHUB_RAW_RESULT, GetGitHubRawResultEventParams({
                    name,
                    data
                }));
            } catch {
